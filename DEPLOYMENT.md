...
This repo is now prepared for a permanent hosted deployment instead of temporary
...
- Domain: Render default domains first, then optional custom domain

The included `render.yaml` can create all three services from one Render
Blueprint.
...
4. Render will create:
   ...
   - `textile-erp-web`
   - `textile-erp-db`
5. After the services are created, update these placeholders if Render assigns
   different URLs:
...
- Migrations run during the Render backend build command.
- The frontend is a single page app; `render.yaml` rewrites all routes to
  `index.html`.
